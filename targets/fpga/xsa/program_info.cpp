...
XsaProgramInfo::XsaProgramInfo(const FlayCompilerResult &compilerResult,
    ...
    : FpgaBaseProgramInfo(compilerResult, std::move(inputBlocks)) {
    ...
    BUG_CHECK(archSpec->getArchVectorSize() == programmableBlocks.size(),
              "The Xsa architecture requires %1% pipes (provided %2% pipes).",
...
std::vector<const IR::Node *> XsaProgramInfo::processDeclaration(
