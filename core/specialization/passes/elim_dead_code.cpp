...
const IR::Node *ElimDeadCode::preorder(IR::P4Table *table) {
    ...
    ASSIGN_OR_RETURN_WITH_MESSAGE(const auto &defaultAction, table->getDefaultAction(), table,
                                  ::error("Table %1% does not have a default action.", table));
    ASSIGN_OR_RETURN_WITH_MESSAGE(const auto &defaultActionCall,
                                  defaultAction.to<IR::MethodCallExpression>(), table,
                                  ::error("%1% is not a method call expression.", defaultAction));
...
const IR::Node *ElimDeadCode::preorder(IR::MethodCallStatement *stmt) {
    ...
    ASSIGN_OR_RETURN_WITH_MESSAGE(const auto &defaultActionCall,
                                  defaultAction.to<IR::MethodCallExpression>(), stmt,
                                  ::error("%1% is not a method call expression.", defaultAction));
