...
IR::Expression *SymbolicEnv::get(const IR::StateVariable &var) const {
...
bool SymbolicEnv::isSymbolicValue(const IR::Node *node) {
    ...
    // Symbolic constants are references to fields under the struct p4t*variables.const.
    if (const auto *member = expr->to<IR::Member>()) {
        return Variables::isSymbolicConst(member);
    }
