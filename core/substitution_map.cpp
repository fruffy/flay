...
Z3SolverSubstitutionMap::Z3SolverSubstitutionMap(Z3Solver &solver, const NodeAnnotationMap &map)
    ...
    for (auto &[node, substitutionExpression] : map.expressionMap()) {
        auto *z3SubstitutionExpression = new Z3SubstitutionExpression(
            ...
            translator.translate(substitutionExpression->originalExpression()));
        emplace(node, z3SubstitutionExpression);
    }
}

std::optional<bool> Z3SolverSubstitutionMap::computeNodeSubstitution(
    const IR::Expression *expression, const ControlPlaneAssignmentSet &constraints) {
    ...
    Z3Translator translator(_solver);
    auto original = translator.translate(it->second->originalExpression());
    auto srcVector = z3::expr_vector(_solver.get().mutableContext());
    auto dstVector = z3::expr_vector(_solver.get().mutableContext());
    for (const auto &constraint : constraints) {
        srcVector.push_back(translator.translate(&constraint.first.get()));
        dstVector.push_back(translator.translate(&constraint.second.get()));
    }

    auto newExpr = original.substitute(srcVector, dstVector).simplify();
...
std::optional<bool> Z3SolverSubstitutionMap::recomputeSubstitution(
    ...
    ControlPlaneAssignmentSet constraints;
    for (const auto &[entityName, controlPlaneConstraint] : controlPlaneConstraints) {
        ...
        constraints.insert(entityConstraints.begin(), entityConstraints.end());
    }
